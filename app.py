...
def generate():
    ...
    try:
        ...
        seed = hash(f"{spotify_user_id}{vibe}{int(time.time() / 3600)}")
        ...
        history_map = {r.spotify_track_id: r.recommended_at for r in recent_rows}

        scored = []

        for track in cache:
            base_score = score_track(track, profile)
            repeat_penalty = apply_repeat_penalty(track["id"], history_map)

            scored.append((track["id"], base_score * repeat_penalty))
