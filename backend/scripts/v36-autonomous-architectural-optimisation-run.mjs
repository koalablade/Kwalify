#!/usr/bin/env node
/**
 * V36 Autonomous Architectural Optimisation — regression vs V35 baseline.
 * Usage: node backend/scripts/v36-autonomous-architectural-optimisation-run.mjs
 */
...
const V35_JSON = resolve(OUT_DIR, "v35-playlist-coverage-architectural-fix.json");
const OUT_JSON = resolve(OUT_DIR, "v36-autonomous-architectural-optimisation.json");
const OUT_MD = resolve(OUT_DIR, "V36_AUTONOMOUS_ARCHITECTURAL_OPTIMISATION.md");
const OUT_LOG = resolve(OUT_DIR, "v36-autonomous-architectural-optimisation-run.log");
...
async function main() {
  log("V36 autonomous architectural optimisation regression starting");
  const commit = execSync("git rev-parse HEAD", { cwd: ROOT, encoding: "utf8" }).trim();
  const v35 = existsSync(V35_JSON) ? JSON.parse(readFileSync(V35_JSON, "utf8")) : null;
        ...
        try {
          const res = await fetch(`${creds.baseUrl}/api/generate?audit=1`, {
            ...
            body: JSON.stringify({
              ...
              requestId: `v36-${spec.id}-${Date.now()}`,
            }),
  ...
  const v35Core = (v35?.rows ?? []).filter((r) => CORE_PROMPTS.some((p) => p.prompt === r.prompt));
  const comparison = v35 ? compareToBaseline(rows, v35.rows ?? []) : [];
  ...
  const payload = {
    ...
    commit,
    baselineCommit: v35?.commit ?? null,
