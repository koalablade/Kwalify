/**
 * V12 live validation — 8 world-critical prompts, KEEP/MAYBE/DROP by thesis authority + full-world proof.
 */
...
function classify(prompt, tracks, httpStatus, coverageLevel) {
  ...
  const opener1Score = profile && opener1
    ...
    : 0;
  const opener1Ok = opener1Score >= 0.85 || (profile && opener1 && scoreTrackWorldIdentity(
    { artistName: opener1.artistName ?? opener1.artist, trackName: opener1.trackName ?? opener1.name },
    profile,
  ) >= 0.8);
  ...
  const allOnWorld =
    profile != null &&
    mapped.length > 0 &&
    mapped.every((t) => scoreTrackWorldIdentity(t, profile) >= 0.5);
  ...
  if (gymHonestMetal) {
    return { verdict: "KEEP", why: "honest gym metal partial, all on-world" };
  }
...
async function main() {
  ...
  const lines = [
    "# V12 Keep Conversion — Human Validation",
    "",
    "Date: 2026-07-28",
    `Base URL: ${creds.baseUrl}`,
    `Commit: ${head} (V12 keep conversion: 80s drive, motorway tail, country, no-rap gym)`,
    "",
    "## Method",
    "- KEEP/MAYBE/DROP judged on cultural world identity + thesis opener + full-playlist sample",
    "- Track 1 must be anchor artist or score >= 0.85 world identity",
    "- Sample tracks 1,3,5,10 + V11 indices must be 80%+ on-world for KEEP",
    "- Target: 8 KEEP / 0 MAYBE / 0 DROP (convert 4 V11 MAYBEs)",
    "",
    "## Results",
    "",
  ...
  for (const row of PROMPTS) {
    const requestId = `v12-${row.id}-${stamp}`;
  ...
  lines.push("");
  lines.push("## V12 vs V11");
  lines.push("- 80s night drive: remix bait blocked, Cure/New Order anchors");
  lines.push("- Motorway: tail strip tracks 5-10 party/phonky without backfill");
  lines.push("- Country cowboy: routes to country_world, rejects indie contamination");
  lines.push("- No rap gym: rock/metal allowed, AC/DC/Metallica openers");
  lines.push("");
  ...
  const outPath = "reports/playlist-evaluation/v12-keep-conversion-2026-07-28.md";
