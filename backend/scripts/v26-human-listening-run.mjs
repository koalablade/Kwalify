#!/usr/bin/env node
/**
 * V26 Human Listening Corrective — fresh Spotify playlists for human re-test.
 *
 * Usage:
 *   node backend/scripts/v26-human-listening-run.mjs [--resume] [--fresh]
 *
...
const OUT_DIR = resolve(ROOT, "reports/playlist-evaluation/v26-human-listening");
const OUT_JSON = resolve(OUT_DIR, "playlists.json");
const OUT_LOG = resolve(OUT_DIR, "run.log");
const FORENSIC_JSON = resolve(ROOT, "v26-human-listening-corrective.json");
...
const PLAYLISTS = [
  { id: "V26-01", name: "Hard techno gym", prompt: "Hard techno gym", kind: "critical" },
  { id: "V26-02", name: "Sunset beach reggae", prompt: "Sunset beach reggae", kind: "critical" },
  { id: "V26-03", name: "Late night UK garage drive", prompt: "Late night UK garage drive", kind: "regression" },
  { id: "V26-04", name: "2000s pop punk gym workout", prompt: "2000s pop punk gym workout", kind: "regression" },
  { id: "V26-05", name: "Pop punk road trip, no Blink-182", prompt: "Pop punk road trip, no Blink-182", kind: "regression" },
  { id: "V26-06", name: "Dad rock BBQ", prompt: "Dad rock BBQ", kind: "regression" },
  { id: "V26-07", name: "Rainy motorway night drive", prompt: "Rainy motorway night drive", kind: "regression" },
  { id: "V26-08", name: "90s grunge driving", prompt: "90s grunge driving", kind: "generalization" },
  { id: "V26-09", name: "Drum and bass gym", prompt: "Drum and bass gym", kind: "generalization" },
  { id: "V26-10", name: "Feel-good reggae", prompt: "Feel-good reggae", kind: "generalization" },
  { id: "V26-11", name: "Melancholy indie", prompt: "Melancholy indie", kind: "generalization" },
  { id: "V26-12", name: "Sunday morning", prompt: "Sunday morning", kind: "generalization" },
  { id: "V26-13", name: "90s hip hop night drive", prompt: "90s hip hop night drive", kind: "generalization" },
  { id: "V26-14", name: "Britpop pub night", prompt: "Britpop pub night", kind: "generalization" },
  { id: "V26-15", name: "Pop punk road trip no Blink", prompt: "Pop punk road trip no Blink-182", kind: "generalization" },
];

/** V25-07 forensic: indie tracks that leaked when reggae_world did not exist. */
const V25_07_BAD_TRACKS = [
  { artistName: "MGMT", trackName: "Electric Feel", spotifyTrackId: "3FtYbEfBanLeBaaYpTrBHP", verdict: "indie_leak_not_reggae" },
  { artistName: "Wallows", trackName: "Are You Bored Yet?", spotifyTrackId: "57RA3JGafJm5zRtKJiKPIm", verdict: "indie_leak_not_reggae" },
  { artistName: "Wallows", trackName: "Remember When", spotifyTrackId: "0Jq5V4Z9y0x0QZ9Q9Q9Q9Q", verdict: "indie_leak_not_reggae" },
  { artistName: "The 1975", trackName: "Chocolate", spotifyTrackId: "18UsAG7SfOQ5sxJEdjAMH0", verdict: "indie_leak_not_reggae" },
  { artistName: "The 1975", trackName: "Somebody Else", spotifyTrackId: "1fDFHXcykq4iw8Gg7s5hG9", verdict: "indie_leak_not_reggae" },
  { artistName: "Mac DeMarco", trackName: "Chamber of Reflection", spotifyTrackId: "0RjsxgVkmjWhxYYH7SZITk", verdict: "indie_leak_not_reggae" },
  { artistName: "Tame Impala", trackName: "The Less I Know The Better", spotifyTrackId: "0Jq5V4Z9y0x0QZ9Q9Q9Q9Q", verdict: "indie_leak_not_reggae" },
  { artistName: "Vampire Weekend", trackName: "A-Punk", spotifyTrackId: "0Jq5V4Z9y0x0QZ9Q9Q9Q9Q", verdict: "indie_leak_not_reggae" },
  { artistName: "Real Estate", trackName: "Talking Backwards", spotifyTrackId: "0Jq5V4Z9y0x0QZ9Q9Q9Q9Q", verdict: "indie_leak_not_reggae" },
];

function writeForensicJson(playlists, blockers, baseUrl) {
  const payload = {
    version: "V26",
    ...
    baseUrl,
    v25Forensic: {
      "V25-07_sunset_beach_reggae": {
        rootCause: "No reggae_world existed; prompt resolved to beach_sunset_world (indie: MGMT, Wallows, 1975)",
        badTrackAnalysis: V25_07_BAD_TRACKS,
        note: "User's 9 Spotify URLs ARE the bad V25-07 tracklist — indie leakage, not reggae.",
      },
      "V25-01_hard_techno_gym": {
        rootCause: "gym_energy_world cultural profile listed Paramore/Fall Out Boy in adjacentArtists and pop punk in instrumentation",
      },
      playlistDepth: {
        rootCause: "world-coverage tier caps (VERY_LOW=5, LOW=12); short playlists = thin genuine pools + honest partial, not padding",
      },
    },
    v26Correctives: [
      "Added reggae_world with explicit genre hard lock",
      "Removed pop-punk adjacency from gym_energy_world",
      "honestPartialCap uses min(genuinePoolCount, tierMax) not 0.4*requested early stop",
    ],
    v26Results: playlists,
    blockers,
  };
  writeFileSync(FORENSIC_JSON, JSON.stringify(payload, null, 2), "utf8");
}
...
async function main() {
  ...
  if (auditOnly) {
    ...
    log(`V26 starting commit=${getHeadCommit()} baseUrl=${baseUrl} mode=${auditOnly ? "audit" : "spotify-create"}`);
  ...
  for (const pl of PLAYLISTS) {
    ...
    const requestId = `v26-${pl.id}-${Date.now()}`;
    try {
      ...
      writeForensicJson(playlists, blockers, baseUrl);
