#!/usr/bin/env node
/**
 * V40 live contract-authoritative retrieval A/B — V37 vs PLAYLIST_CONTRACT_V40=1 only.
 *
 * Usage:
 *   node backend/scripts/v40-live-contract-retrieval-ab.mjs [--skip-a] [--limit N] [--matrix]
 */
...
const OUT_JSON = resolve(OUT_DIR, "v40-live-contract-retrieval-ab.json");
const OUT_MD = resolve(OUT_DIR, "V40_LIVE_CONTRACT_RETRIEVAL_AB.md");
const OUT_LOG = resolve(OUT_DIR, "v40-live-contract-retrieval-ab-run.log");
...
function extractRow(spec, httpStatus, data, arm) {
  ...
  const v40 = data.playlistContractV40 ?? null;
  const v40Retrieval = v40?.retrieval?.v40 ?? v40?.retrieval ?? null;
  ...
  return {
    ...
    v40: v40
      ? {
          deferHardLock: v40.deferHardLock ?? worldGate?.deferHardLock ?? false,
          retrievalAuthority: v40.retrievalAuthority ?? null,
          eligibleAfterHardFilters: v40Retrieval?.eligibleAfterHardFilters ?? null,
          inputCount: v40Retrieval?.inputCount ?? null,
          outputCount: v40Retrieval?.outputCount ?? null,
          genreExpectations: v40Retrieval?.genreExpectations ?? null,
          tensionPreservation: v40Retrieval?.tensionPreservation ?? null,
        }
      : null,
    funnel,
  ...
  try {
    const res = await fetch(`${baseUrl}/api/generate?audit=1`, {
      ...
      body: JSON.stringify({
        ...
        auditMode: true,
        spotifyUserId: USER,
        requestId: `v40-ab-${arm}-${id}-${Date.now()}`,
      }),
...
async function runArm(arm, prompts, baseUrl, token) {
  ...
  for (const spec of prompts) {
    ...
    try {
      ...
      rows.push(row);
      log(`  → del=${row.funnel.delivered} ret=${row.funnel.retrieval} v40=${row.v40?.retrievalAuthority ?? "off"} eligible=${row.v40?.eligibleAfterHardFilters ?? "n/a"}`);
    } catch (err) {
...
function comparePrompt(aRow, bRow) {
  ...
  return {
    ...
    deferHardLock: bRow.v40?.deferHardLock ?? bRow.worldGate?.deferHardLock ?? false,
    ...
    poolChanged: (bRow.funnel?.retrieval ?? 0) !== (aRow.funnel?.retrieval ?? 0)
      || (bRow.v40?.eligibleAfterHardFilters != null && bRow.v40.eligibleAfterHardFilters !== aRow.funnel?.library)
      || (bRow.v40?.outputCount != null && bRow.v40.outputCount !== aRow.funnel?.retrieval),
    bV40Eligible: bRow.v40?.eligibleAfterHardFilters ?? null,
    bV40Output: bRow.v40?.outputCount ?? null,
  };
}
...
function renderMd(payload) {
  const L = [];
  L.push("# V40 Live Contract-Authoritative Retrieval A/B");
  L.push("");
  ...
  L.push("| B (V40) | `PLAYLIST_CONTRACT_V40=1` only |");
  L.push("");
...
async function main() {
  ...
  mkdirSync(OUT_DIR, { recursive: true });
  appendFileSync(OUT_LOG, `\n=== V40 A/B run ${new Date().toISOString()} ===\n`, "utf8");

  log("Building...");
  execSync("npm run build", { cwd: ROOT, stdio: "inherit" });

  const commit = execSync("git rev-parse HEAD", { cwd: ROOT, encoding: "utf8" }).trim();
  const uncommittedV40 = existsSync(join(ROOT, "backend/core/playlist-contract/contract-authoritative-retrieval.ts"));
  ...
  try {
    if (!skipA) {
      ...
      log(`Arm A flags: all contract flags off`);
    ...
    log("=== ARM B: V40 CONTRACT_RETRIEVAL_AUTHORITY only ===");
    const spawnedB = await spawnApiServer({ PLAYLIST_CONTRACT_V40: "1" }, creds.token, { reuseIfReady: false });
    bServer = spawnedB.server;
    log(`Arm B flags: V40=${spawnedB.env?.PLAYLIST_CONTRACT_V40}`);
  ...
  let recommendation = "KEEP OFF — do not promote V40";
  ...
  if (hypothesisSupported) {
    ...
    nextStep = "Investigate composition/V3 bottleneck — retrieval now supplies better material.";
  } else if (poolChangedDeferred.length === 0 && deferred.length > 0) {
    recommendation = "REDESIGN V40 — contract retrieval did not change pool";
    nextStep = "Verify contract-authoritative branch is reached at retrieveScoringCandidates entry.";
  ...
  const payload = {
    ...
    configuration: {
      armA: { PLAYLIST_CONTRACT_V40: "0", PLAYLIST_CONTRACT_WORLD_GATE: "0", PLAYLIST_CONTRACT_RETRIEVAL: "0", PLAYLIST_CONTRACT_SHADOW: "0" },
      armB: { PLAYLIST_CONTRACT_V40: "1", PLAYLIST_CONTRACT_WORLD_GATE: "0", PLAYLIST_CONTRACT_RETRIEVAL: "0", PLAYLIST_CONTRACT_SHADOW: "0" },
    },
    arms: { a: aRows, b: bRows },
    comparisons,
    controlComparisons,
    summary: {
      deferCount: bRows.filter((r) => r.v40?.deferHardLock || r.worldGate?.deferHardLock).length,
