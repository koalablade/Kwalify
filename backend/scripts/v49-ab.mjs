#!/usr/bin/env node
/**
 * V49 human-quality A/B — baseline vs candidate with independent verifier + full tracklists.
 *
 * Usage:
 *   node backend/scripts/v49-ab.mjs [--baseline 3606ce9] [--candidate HEAD] [--limit N] [--skip-baseline] [--candidate-only]
 */
...
const OUT_JSON = resolve(OUT_DIR, "v49-ab-validation.json");
const OUT_MD = resolve(OUT_DIR, "V49_AB_VALIDATION.md");
const OUT_LOG = resolve(OUT_DIR, "v49-ab-run.log");
...
const COMPOUND_PROBES = [
  { id: "V49-C01", category: "compound", prompt: "sad party bangers" },
  { id: "V49-C02", category: "compound", prompt: "party but not cheesy" },
  { id: "V49-C03", category: "compound", prompt: "party but restrained" },
  { id: "V49-C04", category: "compound", prompt: "melancholic and danceable" },
  { id: "V49-C05", category: "compound", prompt: "energetic but not cheesy" },
  { id: "V49-C06", category: "compound", prompt: "chilled but not boring" },
  { id: "V49-C07", category: "compound", prompt: "dark and danceable" },
  { id: "V49-C08", category: "compound", prompt: "warm and melancholic" },
  { id: "V49-C09", category: "compound", prompt: "emotional but upbeat" },
  { id: "V49-C10", category: "compound", prompt: "aggressive but controlled" },
  { id: "V49-C11", category: "compound", prompt: "nostalgic driving" },
  { id: "V49-C12", category: "compound", prompt: "90s indie road trip nostalgia" },
];

const VAGUE_PROBES = [
  { id: "V49-V01", category: "vague", prompt: "late night drive" },
  { id: "V49-V02", category: "vague", prompt: "something nostalgic" },
  { id: "V49-V03", category: "vague", prompt: "rainy Sunday" },
  { id: "V49-V04", category: "vague", prompt: "summer evening" },
  { id: "V49-V05", category: "vague", prompt: "long drive" },
  { id: "V49-V06", category: "vague", prompt: "old songs that hit different" },
  { id: "V49-V07", category: "vague", prompt: "feeling weird" },
  { id: "V49-V08", category: "vague", prompt: "quiet night" },
  { id: "V49-V09", category: "vague", prompt: "cozy Sunday morning" },
  { id: "V49-V10", category: "vague", prompt: "lo-fi study" },
];

const CONTROL_PROBES = [
  { id: "V49-CTL01", category: "control", prompt: "dad rock BBQ" },
  { id: "V49-CTL02", category: "control", prompt: "sunset beach reggae" },
  { id: "V49-CTL03", category: "control", prompt: "2000s pop punk gym workout" },
  { id: "V49-CTL04", category: "control", prompt: "cozy sunday morning coffee" },
  { id: "V49-CTL05", category: "control", prompt: "lo-fi study focus" },
];
...
function extractRow(spec, httpStatus, data, arm, sha, evaluateHumanCurationScore, verifyIndependentHumanQuality) {
  ...
  return {
    ...
    independentVerifier: {
      ...
      roiFailures: verifier.roiFailures.slice(0, 5),
  ...
  try {
    const res = await fetch(`${baseUrl}/api/generate?audit=1`, {
      ...
      body: JSON.stringify({
        ...
        auditMode: true,
        spotifyUserId: USER,
        requestId: `v49-ab-${arm}-${id}-${Date.now()}`,
      }),
  ...
  for (const spec of prompts) {
    ...
    for (let attempt = 0; attempt < 3; attempt += 1) {
      try {
        ...
        rows.push(row);
        log(
          `  → del=${row.delivered} proxy=${row.humanCuratorProxy} verifier=${row.independentVerifier.playlistVerdict} score=${row.humanScore.totalScore} spam=${row.spamHits} misfits=${row.independentVerifier.misfitCount}`,
        );
        lastErr = null;
        break;
      } catch (err) {
...
function formatPlaylistSection(row) {
  ...
  lines.push(
    `- **Arm:** ${row.arm} | **Delivered:** ${row.delivered} | **Human-curator proxy:** ${row.humanCuratorProxy} | **Independent verifier:** ${row.independentVerifier?.playlistVerdict ?? "?"}`,
  );
  lines.push(
    `- **Score:** ${row.humanScore?.totalScore ?? 0}/100 | Save: ${row.humanScore?.wouldSave ?? "?"} | Misfits: ${row.independentVerifier?.misfitCount ?? 0} | Spam: ${row.spamHits ?? 0}`,
  );
  if (row.independentVerifier?.failureReasons?.length) {
    lines.push(`- **Verifier failures:** ${row.independentVerifier.failureReasons.join("; ")}`);
  }
  if (row.verifierMisfits?.length) {
    lines.push(`- **Verifier misfits:** ${row.verifierMisfits.join("; ")}`);
  }
  lines.push("");
  ...
  for (const t of row.deliveredTracks ?? []) {
    ...
    lines.push(`${t.position}. ${t.artist} — ${t.track} [${flag}]`);
  }
...
async function main() {
  ...
  appendFileSync(
    OUT_LOG,
    `\n=== V49 A/B ${new Date().toISOString()} baseline=${baseline} candidate=${candidate} ===\n`,
    "utf8",
  );
  ...
  const baselineWorktree = resolve(ROOT, "..", `Kwalify-v49-baseline-${baselineSha.slice(0, 7)}`);
  ...
  const md = [];
  md.push(`# V49 Human-Quality A/B Validation`);
  md.push("");
  md.push(`**Baseline:** \`${baselineSha}\``);
  md.push(`**Candidate:** \`${candidateSha}\``);
  md.push(`**Verdict:** ${verdict.verdict}`);
  md.push(
    `**Strong/Mixed/Weak (candidate verifier):** ${verdict.strongCount}/${comparisons.filter((c) => c.candidateVerifier === "mixed").length}/${verdict.weakCount}`,
  );
  md.push("");
  md.push("## Global ROI failures (candidate)");
  md.push("");
  for (const roi of globalRoi.slice(0, 8)) {
  ...
  for (const c of comparisons) {
    md.push(
      `- **${c.prompt}** [${c.category}]: ${verdict.perPrompt[c.prompt]} | verifier ${c.baselineVerifier}→${c.candidateVerifier} | del ${c.baselineDelivered}→${c.candidateDelivered} | misfits ${c.baselineMisfits}→${c.candidateMisfits}`,
    );
  }
  md.push("");
