#!/usr/bin/env node
/**
 * V46 validation A/B — V45 baseline (45f92b2) vs V46 semantic-moment candidate.
 *
 * Usage:
 *   node backend/scripts/v46-ab.mjs [--baseline 45f92b2] [--candidate HEAD] [--limit N] [--skip-baseline]
 */
...
const OUT_JSON = resolve(OUT_DIR, "v46-ab-validation.json");
const OUT_MD = resolve(OUT_DIR, "V46_AB_VALIDATION.md");
const OUT_LOG = resolve(OUT_DIR, "v46-ab-run.log");
...
const VAGUE_PROBES = [
  { id: "V46-V01", category: "vague", prompt: "late night drive" },
  { id: "V46-V02", category: "vague", prompt: "something nostalgic" },
  { id: "V46-V03", category: "vague", prompt: "rainy Sunday" },
  { id: "V46-V04", category: "vague", prompt: "summer evening" },
  { id: "V46-V05", category: "vague", prompt: "feeling weird" },
  { id: "V46-V06", category: "vague", prompt: "old songs that hit different" },
  { id: "V46-V07", category: "vague", prompt: "long drive" },
];

const TIGHT_PROBES = [
  { id: "V46-T01", category: "tension", prompt: "sad party bangers" },
  { id: "V46-T02", category: "tension", prompt: "party but not cheesy" },
  { id: "V46-T03", category: "tension", prompt: "energetic but not cheesy" },
  { id: "V46-T04", category: "tension", prompt: "chilled but not boring" },
  { id: "V46-T05", category: "tension", prompt: "dark and danceable" },
  { id: "V46-T06", category: "tension", prompt: "warm and melancholic" },
  { id: "V46-T07", category: "tension", prompt: "emotional but upbeat" },
  { id: "V46-T08", category: "tension", prompt: "aggressive but controlled" },
  { id: "V46-T09", category: "tension", prompt: "something nostalgic for driving" },
  { id: "V46-T10", category: "tension", prompt: "90s indie road trip nostalgia" },
];

const CONTROL_PROBES = [
  { id: "V46-CTL01", category: "control", prompt: "dad rock BBQ" },
  { id: "V46-CTL02", category: "control", prompt: "cozy sunday morning coffee" },
  { id: "V46-CTL03", category: "control", prompt: "sunset beach reggae" },
  { id: "V46-CTL04", category: "control", prompt: "2000s pop punk gym workout" },
  { id: "V46-CTL05", category: "control", prompt: "lo-fi study focus" },
];
  ...
  try {
    const res = await fetch(`${baseUrl}/api/generate?audit=1`, {
      ...
      body: JSON.stringify({
        ...
        auditMode: true,
        spotifyUserId: USER,
        requestId: `v46-ab-${arm}-${id}-${Date.now()}`,
      }),
...
async function main() {
  ...
  mkdirSync(OUT_DIR, { recursive: true });
  appendFileSync(OUT_LOG, `\n=== V46 A/B ${new Date().toISOString()} baseline=${baseline} candidate=${candidate} ===\n`, "utf8");
  ...
  const baselineWorktree = resolve(ROOT, "..", `Kwalify-v46-baseline-${baselineSha.slice(0, 7)}`);
  ...
  writeFileSync(
    OUT_MD,
    `# V46 A/B Validation\n\n**Baseline (V45):** \`${baselineSha}\`\n**Candidate (V46):** \`${candidateSha}\`\n**Verdict:** ${verdict.verdict}\n\n` +
