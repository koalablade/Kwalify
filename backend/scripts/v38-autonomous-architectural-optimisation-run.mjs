#!/usr/bin/env node
/**
 * V38 Autonomous Architectural Optimisation — regression vs V37 baseline.
 * Usage: node backend/scripts/v38-autonomous-architectural-optimisation-run.mjs
 */
...
const V37_JSON = resolve(OUT_DIR, "v37-autonomous-architectural-optimisation.json");
const OUT_JSON = resolve(OUT_DIR, "v38-autonomous-architectural-optimisation.json");
const OUT_MD = resolve(OUT_DIR, "V38_AUTONOMOUS_ARCHITECTURAL_OPTIMISATION.md");
const OUT_LOG = resolve(OUT_DIR, "v38-autonomous-architectural-optimisation-run.log");
...
async function main() {
  log("V38 autonomous architectural optimisation regression starting");
  const commit = execSync("git rev-parse HEAD", { cwd: ROOT, encoding: "utf8" }).trim();
  const v37 = existsSync(V37_JSON) ? JSON.parse(readFileSync(V37_JSON, "utf8")) : null;
        ...
        try {
          const res = await fetch(`${creds.baseUrl}/api/generate?audit=1`, {
            ...
            body: JSON.stringify({
              ...
              requestId: `v38-${spec.id}-${Date.now()}`,
            }),
  ...
  const v37Core = (v37?.rows ?? []).filter((r) => CORE_PROMPTS.some((p) => p.prompt === r.prompt));
  const comparison = v37 ? compareToBaseline(rows, v37.rows ?? []) : [];
  ...
  const payload = {
    ...
    commit,
    baselineCommit: v37?.commit ?? null,
  ...
  log(`Wrote ${OUT_MD}`);
  log(`Wrote ${OUT_JSON}`);
  log(`Done. Avg delivered: ${aggregate.after.all.avgDelivered?.toFixed(1)} (V37 baseline: ${aggregate.before.avgDelivered?.toFixed(1) ?? "n/a"})`);
}
