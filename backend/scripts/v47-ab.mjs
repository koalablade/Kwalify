#!/usr/bin/env node
/**
 * V47 validation A/B — V46 baseline (a151cd8) vs V47 candidate.
 *
 * Usage:
 *   node backend/scripts/v47-ab.mjs [--baseline a151cd8] [--candidate HEAD] [--limit N] [--skip-baseline]
 */
...
const OUT_JSON = resolve(OUT_DIR, "v47-ab-validation.json");
const OUT_MD = resolve(OUT_DIR, "V47_AB_VALIDATION.md");
const OUT_LOG = resolve(OUT_DIR, "v47-ab-run.log");
...
const VAGUE_PROBES = [
  { id: "V47-V01", category: "vague", prompt: "late night drive" },
  { id: "V47-V02", category: "vague", prompt: "something nostalgic" },
  { id: "V47-V03", category: "vague", prompt: "rainy Sunday" },
  { id: "V47-V04", category: "vague", prompt: "summer evening" },
  { id: "V47-V05", category: "vague", prompt: "feeling weird" },
  { id: "V47-V06", category: "vague", prompt: "old songs that hit different" },
  { id: "V47-V07", category: "vague", prompt: "long drive" },
  { id: "V47-V08", category: "vague", prompt: "quiet night" },
];

const TIGHT_PROBES = [
  { id: "V47-T01", category: "tension", prompt: "sad party bangers" },
  { id: "V47-T02", category: "tension", prompt: "party but not cheesy" },
  { id: "V47-T03", category: "tension", prompt: "energetic but not cheesy" },
  { id: "V47-T04", category: "tension", prompt: "chilled but not boring" },
  { id: "V47-T05", category: "tension", prompt: "dark and danceable" },
  { id: "V47-T06", category: "tension", prompt: "warm and melancholic" },
  { id: "V47-T07", category: "tension", prompt: "emotional but upbeat" },
  { id: "V47-T08", category: "tension", prompt: "aggressive but controlled" },
  { id: "V47-T09", category: "tension", prompt: "something nostalgic for driving" },
  { id: "V47-T10", category: "tension", prompt: "90s indie road trip nostalgia" },
  { id: "V47-T11", category: "tension", prompt: "melancholic+danceable" },
  { id: "V47-T12", category: "tension", prompt: "party+restrained" },
];

const CONTROL_PROBES = [
  { id: "V47-CTL01", category: "control", prompt: "dad rock BBQ" },
  { id: "V47-CTL02", category: "control", prompt: "cozy sunday morning coffee" },
  { id: "V47-CTL03", category: "control", prompt: "sunset beach reggae" },
  { id: "V47-CTL04", category: "control", prompt: "2000s pop punk gym workout" },
  { id: "V47-CTL05", category: "control", prompt: "lo-fi study focus" },
];

const ALL_PROMPTS = [...VAGUE_PROBES, ...TIGHT_PROBES, ...CONTROL_PROBES];
  ...
  try {
    const res = await fetch(`${baseUrl}/api/generate?audit=1`, {
      ...
      body: JSON.stringify({
        ...
        auditMode: true,
        spotifyUserId: USER,
        requestId: `v47-ab-${arm}-${id}-${Date.now()}`,
      }),
...
async function runArm(arm, sha, prompts, baseUrl, token) {
  ...
  for (const spec of prompts) {
    ...
    try {
      ...
      rows.push(row);
      log(`  → del=${row.delivered} ok=${row.success} rebal=${row.rebalanced} spam=${row.spamHits} compound=${row.compoundHits} party=${row.partyEnergyHits} mel=${row.melancholyHits}`);
    } catch (err) {
...
async function main() {
  ...
  mkdirSync(OUT_DIR, { recursive: true });
  appendFileSync(OUT_LOG, `\n=== V47 A/B ${new Date().toISOString()} baseline=${baseline} candidate=${candidate} ===\n`, "utf8");
  ...
  const baselineWorktree = resolve(ROOT, "..", `Kwalify-v47-baseline-${baselineSha.slice(0, 7)}`);
  ...
  writeFileSync(
    OUT_MD,
    `# V47 A/B Validation\n\n**Baseline (V46):** \`${baselineSha}\`\n**Candidate (V47):** \`${candidateSha}\`\n**Verdict:** ${verdict.verdict}\n\n` +
      comparisons.map((c) => `- ${c.prompt}: ${verdict.perPrompt[c.prompt]} (del ${c.baselineDelivered}→${c.candidateDelivered}, spam ${c.baselineSpam}→${c.candidateSpam}, compound ${c.baselineCompound}→${c.candidateCompound})`).join("\n"),
    "utf8",
  );
  log(`Wrote ${OUT_JSON}`);
