...
export async function createSpotifyPlaylist(
  ...
  opts?: {
    ...
    /** Defaults to the production Kwalify description. QA tooling may override. */
    description?: string;
  }
): Promise<{
  id: string;
    ...
    const playlistResponse = await spotifyRequest<any>(
      {
        ...
        data: {
          name,
          public: false,
          description: opts?.description ?? `Generated by K_WALAH — your emotional AI DJ`,
        },
        headers: {
...
/** Unfollow/delete a playlist for the current user. Used by QA cleanup only. */
...
/** Add URIs one-by-one so QA can record per-track failures without skipping silently. */
export async function addSpotifyPlaylistUrisOneByOne(
  accessToken: string,
  playlistId: string,
  uris: string[],
  userKey?: string,
): Promise<{ added: string[]; failed: Array<{ uri: string; reason: string }> }> {
  const added: string[] = [];
  ...
  for (const uri of uris) {
    try {
      ...
      const { status, message } = spotifyErrorFields(err);
      failed.push({ uri, reason: `${status ?? "error"}: ${message ?? "add failed"}` });
    }
  }
  return { added, failed };
}
